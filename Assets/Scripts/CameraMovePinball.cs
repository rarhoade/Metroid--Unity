...
public class CameraMovePinball : MonoBehaviour
{
    ...
    private Vector2[] roomMins = { new Vector2(0, 15), new Vector2(78, 15), new Vector2(96, 15), };
    private Vector2[] roomMaxs = { new Vector2(77, 29), new Vector2(95, 29), new Vector2(111, 241) };

    private Vector2[] minBounds = { new Vector2(21.5f, 22), new Vector2(85.5f, 22), new Vector2(103.5f, 22) };
    private Vector2[] maxBounds = { new Vector2(69.5f, 22), new Vector2(87.5f, 22), new Vector2(103.5f, 233) };
    ...
    void Start()
    {
        ...
        player.GetComponent<PlayerState>().checkStuff = false;
        mapInit();
    }
    ...
    void FixedUpdate()
    {
        ...
        if (!transitioning)
        {
            ...
            {
                if (roomMins[i].x < player.transform.position.x && player.transform.position.x < roomMaxs[i].x &&
                    roomMins[i].y < player.transform.position.y && player.transform.position.y < roomMaxs[i].y)
                {
                    roomNum = i;
                }
            }
